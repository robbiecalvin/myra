...
import type { ParsedIntent, RecommendationResponse, WineRecord } from "../types/recommendation.js";

const formatPrice = (price: number): string => `$${price}`;

const scoreWine = (wine: WineRecord, intent: ParsedIntent): number => {
  let score = 0;
...
const reasonForWine = (wine: WineRecord, intent: ParsedIntent): string => {
...
export const recommend = (intent: ParsedIntent): RecommendationResponse => {
  const ranked = [...wines]
